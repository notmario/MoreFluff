...
SMODS.DrawStep({
	...
	order = 26,
	func = function(self)
		if not G.mf_loyalty_spr then
			G.mf_loyalty_spr = Sprite(0, 0, 71, 95, G.ASSET_ATLAS["mf_loyalty"], { x = 0, y = 0 }) -- im dumb and stupide
		end
		...
		if self.config.center.loyalty_atlas then
			...
			if not G.mf_custom_loyalty[atlas] then
				G.mf_custom_loyalty[atlas] = Sprite(0, 0, 71, 95, G.ASSET_ATLAS[atlas], { x = 0, y = 0 })
			end
...
function G.UIDEF.use_and_sell_buttons(card)
	...
	if (card.area == G.jokers and G.jokers and is_planeswalker(card)) and not card.debuff then
		...
		for i, cost in ipairs(costs) do
							...
							nodes = {

								{
									...
									nodes = {
										{
											n = G.UIT.T,
											config = { text = text, colour = G.C.WHITE, scale = 0.3, shadow = true },
										},
									},
								},
							},
			...
			nodesthing[#nodesthing + 1] = { n = G.UIT.R, config = { align = "cl" }, nodes = {
				ability,
			} }
		end
...
function can_loyalty(joker, ability_number)
	if
		...
		or (G.GAME.STOP_USE and G.GAME.STOP_USE > 0) --or
	...
	return can_pay_cost
		and (joker.ability.extra.uses ~= 0 or not joker.config.center.default_loyalty_effects)
		and joker.config.center.can_loyalty(joker, ability_number)
end
