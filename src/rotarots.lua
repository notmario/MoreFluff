...
function Node:collides_with_point(point, ...)
	if self.config and self.config.center and self.config.center.mf_rotate_by then
		self.T.r = self.T.r + self.config.center.mf_rotate_by
	end
...
SMODS.DrawStep {
    ...
    order = 50,
    func = function(card)
        if card.config.center.key == "c_mf_rot_soul" and (card.config.center.discovered or card.bypass_discovery_center) then
            local scale_mod = 0.05 + 0.05 * math.sin(1.8 * G.TIMERS.REAL) +
                0.07 * math.sin((G.TIMERS.REAL - math.floor(G.TIMERS.REAL)) * math.pi * 14) *
                (1 - (G.TIMERS.REAL - math.floor(G.TIMERS.REAL))) ^ 3
            local rotate_mod = 0.1 * math.sin(1.219 * G.TIMERS.REAL) + G.TIMERS.REAL / 8 +
